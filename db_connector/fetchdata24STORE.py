...
# 🔹 Ahora sí se puede importar sin error
...
# ---------------------------------------------------------
# 🔑 ProviderID de 24 Store
# ---------------------------------------------------------
STORE24_PROVIDER_ID = 427
...
# ---------------------------------------------------------
# 🚀 Ejecución del SP (24 Store)
# ---------------------------------------------------------
def fetch_24store_products(catalog_id: int) -> pd.DataFrame:
    """
    Ejecuta el SP GetValidSKUsByCatalogIDs para obtener los productos
    correspondientes al ProviderID de 24 Store (427).
    """
    print(
        f"\n🔍 Ejecutando SP '{STORED_PROC}' "
        f"para ProviderID={STORE24_PROVIDER_ID}, CatalogID={catalog_id}..."
    )

    query = f"EXEC dbo.{STORED_PROC} @catalogIDs = ?, @vendorid = ?"

    conn = get_connection()
    df = pd.read_sql(query, conn, params=(str(catalog_id), STORE24_PROVIDER_ID))
    conn.close()
...
def save_dataframe(df: pd.DataFrame):
    """
    Guarda el DataFrame como ProductosPatagonia_<timestamp>.csv
    en /DataStorage (raíz del proyecto)
    """
...
if __name__ == "__main__":
    df = fetch_24store_products(CATALOG_ID)
