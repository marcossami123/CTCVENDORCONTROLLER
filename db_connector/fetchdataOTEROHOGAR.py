...
# ProviderID Otero Hogar
OTERO_PROVIDER_ID = 506
...
# 🚀 Ejecución del SP
# ---------------------------------------------------------
def fetch_otero_products(catalog_id: int) -> pd.DataFrame:
    """
    Ejecuta el SP GetValidSKUsByCatalogIDs para obtener los productos
    correspondientes al ProviderID de Otero Hogar (506).
    """
    print(f"\n🔍 Ejecutando SP '{STORED_PROC}' para ProviderID={OTERO_PROVIDER_ID}, CatalogID={catalog_id}...")
    query = f"EXEC dbo.{STORED_PROC} @catalogIDs = ?, @vendorid = ?"

    conn = get_connection()
    df = pd.read_sql(query, conn, params=(str(catalog_id), OTERO_PROVIDER_ID))
    conn.close()
...
def save_dataframe(df: pd.DataFrame):
    """
    Guarda el DataFrame como ProductosOtero_<timestamp>.csv
...
if __name__ == "__main__":
    df = fetch_otero_products(CATALOG_ID)
