...
# ---------------------------------------------------------
# 🚀 Ejecución del SP (Ceven)
# ---------------------------------------------------------
def fetch_ceven_products(catalog_id: int) -> pd.DataFrame:
    """
    Ejecuta el SP GetValidSKUsByCatalogIDs para obtener los productos
    correspondientes al ProviderID de Ceven (431).
    """
    print(
        f"\n🔍 Ejecutando SP '{STORED_PROC}' "
        f"para ProviderID={CEVEN_PROVIDER_ID}, CatalogID={catalog_id}..."
    )

    query = f"EXEC dbo.{STORED_PROC} @catalogIDs = ?, @vendorid = ?"

    conn = get_connection()
    df = pd.read_sql(query, conn, params=(str(catalog_id), CEVEN_PROVIDER_ID))
    conn.close()
...
if __name__ == "__main__":
    df = fetch_ceven_products(CATALOG_ID)
