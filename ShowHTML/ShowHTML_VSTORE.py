...
# ---------------------------------------------------------
# 🔍 Obtener el CSV con los links de Vstore
# ---------------------------------------------------------
def get_latest_vstore_links_csv() -> Path:
    files = sorted(
        [f for f in DATA_DIR.glob("Vstore_Links*.csv")],
        key=lambda x: x.stat().st_mtime,
        reverse=True
    )

    if not files:
        raise FileNotFoundError("❌ No se encontró ningún archivo Vstore_Links*.csv")
...
# ---------------------------------------------------------
def generate_vstore_html_dataset() -> pd.DataFrame:
    csv_path = get_latest_vstore_links_csv()
    df = pd.read_csv(csv_path, dtype=str)

    if "URL_VSTORE" not in df.columns:
        raise KeyError("❌ La columna URL_VSTORE no existe en el archivo de links.")

    html_list = []

    print("\n🔍 Iniciando descarga de HTML para cada link...\n")

    for idx, url in enumerate(df["URL_VSTORE"], start=1):
        print(f"➡ ({idx}/{len(df)}) Obteniendo HTML de: {url}")

        html = fetch_html(url)
        html_list.append(html)

        sleep(0.3)  # Para evitar bloqueos anti-scraping
...
def save_html_dataset(df: pd.DataFrame):
    ...
    output_file = DATA_DIR / f"Vstore_HTML_{timestamp}.csv"
...
if __name__ == "__main__":
    df_html = generate_vstore_html_dataset()
