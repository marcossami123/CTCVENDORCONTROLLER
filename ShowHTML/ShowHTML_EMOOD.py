...
# ---------------------------------------------------------
# 🔍 Obtener el CSV con los links de Emood
# ---------------------------------------------------------
def get_latest_emood_links_csv() -> Path:
    files = sorted(
        [f for f in DATA_DIR.glob("Emood_Links*.csv")],
        key=lambda x: x.stat().st_mtime,
        reverse=True
    )

    if not files:
        raise FileNotFoundError("❌ No se encontró ningún archivo Emood_Links*.csv")
...
# ---------------------------------------------------------
def generate_emood_html_dataset() -> pd.DataFrame:
    csv_path = get_latest_emood_links_csv()
    df = pd.read_csv(csv_path, dtype=str)

    if "URL_EMOOD" not in df.columns:
        raise KeyError("❌ La columna URL_EMOOD no existe en el archivo de links.")

    html_list = []

    print("\n🔍 Iniciando descarga de HTML para cada link...\n")

    for idx, url in enumerate(df["URL_EMOOD"], start=1):
        print(f"➡ ({idx}/{len(df)}) Obteniendo HTML de: {url}")

        html = fetch_html(url)
        html_list.append(html)

        sleep(0.3)  # evita rate-limits
...
def save_html_dataset(df: pd.DataFrame):
    ...
    output_file = DATA_DIR / f"Emood_HTML_{timestamp}.csv"
...
if __name__ == "__main__":
    df_html = generate_emood_html_dataset()
